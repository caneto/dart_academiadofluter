void main() {
  ...
  print(
      '\n1 - Apresente os pacientes com menos de 20 anos e print o nome deles');
  for (final paciente in pacientes) {
    final pacientesDados = paciente.split('|');
    final idade = int.tryParse(pacientesDados[1]) ?? 0;
    if (idade < 20) {
  ...
  // PACIENTES QUE NAO MORAM EM SP.
  print('\n3 - Apresente a quantidade de pacientes que não moram em SP');
  listaPacientesSP = pacientes
      .where((element) => element.split("|").last.toLowerCase() != 'sp');

  print('Quantidade de pessoas que não moram em SP: ${listaPacientesSP.length}');
